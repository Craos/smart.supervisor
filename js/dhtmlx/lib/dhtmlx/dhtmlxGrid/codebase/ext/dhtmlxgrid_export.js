 ...
 */
dhtmlXGridObject.prototype.toPDF = function (q, k, l, a, g, c) {
    function r(h) {
        ...
        var d = "<rows profile='" + h + "'";
        l && (d += " header='" + l + "'");
        a && (d += " footer='" + a + "'");
        d += "><head>" + b._serialiseExportConfig(i).replace(/^<head/, "<columns").replace(/head>$/, "columns>");
        for (e = 2; e < b.hdr.rows.length; e++) {
            for (var c = 0, g = b.hdr.rows[e], j = "",
                     f = 0; f < b._cCount; f++)if (b._srClmn && !b._srClmn[f] || b._hrrar[f] && (!b._fake || f >= b._fake.hdrLabels.length))c++; else {
                var k = i[e][f], m = k[0] && k[0] > 1 ? ' colspan="' + k[0] + '" ' : "";
                k[1] && k[1] > 1 && (m += ' rowspan="' + k[1] + '" ', c = -1);
                for (var n = "", p = 0; p < g.cells.length; p++)if (g.cells[p]._cellIndexS == f) {
    ...
    function m() {
        var h = [];
        if (g)for (var i = 0; i < g.length; i++)h.push(w(b.getRowIndex(g[i]))); else for (i = 0; i < b.getRowsNum(); i++)h.push(w(i));
        return h.join("\n")
    }
    ...
    function o() {
        ...
        for (var i = 1; i < b.ftr.rows.length; i++) {
            h.push("<columns>");
            for (var e = b.ftr.rows[i], f = 0; f < b._cCount; f++)if (!b._srClmn || b._srClmn[f])if (!b._hrrar[f] || b._fake && !(f >= b._fake.hdrLabels.length)) {
    ...
    function h(b, a) {
        return(window.getComputedStyle ? window.getComputedStyle(b, null)[a] : b.currentStyle ? b.currentStyle[a] :
            null) || ""
    }

    function w(a) {
        if (!b.rowsBuffer[a])return"";
        var c = b.render_row(a);
        if (c.style.display == "none")return"";
        for (var e = b.isTreeGrid() ? ' level="' + b.getLevel(c.idd) + '"' : "", f = "<row" + e + ">", g = 0; g < b._cCount; g++)if ((!b._srClmn || b._srClmn[g]) && (!b._hrrar[g] || b._fake && g < b._fake.hdrLabels.length)) {
            var d = b.cells(c.idd, g);
            if (x) {
                var k = h(d.cell, "color"), j = h(d.cell, "backgroundColor"), l = h(d.cell, "font-weight") || h(d.cell, "fontWeight"), m = h(d.cell, "font-style") || h(d.cell, "fontStyle"), o = h(d.cell, "text-align") || h(d.cell,
                    "textAlign"), n = h(d.cell, "font-family") || h(d.cell, "fontFamily");
                if (j == "transparent" || j == "rgba(0, 0, 0, 0)")j = "rgb(255,255,255)";
    ...
    var v = "form_" + b.uid();
    t.innerHTML =
        '<form id="' + v + '" method="post" action="' + q + '" accept-charset="utf-8"  enctype="application/x-www-form-urlencoded"' + this.target + '><input type="hidden" name="grid_xml" id="grid_xml"/> </form>';
    document.getElementById(v).firstChild.value = encodeURIComponent(r(k).replace("\u2013", "-") + m() + y());
    document.getElementById(v).submit();
    t.parentNode.removeChild(t);
    b = null;
    j && (j.el.parentNode.className += " rowselected", j.el.className += " cellselected");
    j = null
};
dhtmlXGridObject.prototype._serialiseExportConfig = function (q) {
    ...
    for (var l = "<head>", a = 0; a < this.hdr.rows[0].cells.length; a++)if (!this._srClmn || this._srClmn[a])if (!this._hrrar[a] || this._fake && !(a >= this._fake.hdrLabels.length)) {
        ...
        l += this._asCDATA ? "<![CDATA[" + this.getHeaderCol(a) + "]]\>" : this.getHeaderCol(a);
        var m = this.getCombo(a);
        if (m)for (var o = 0; o < m.keys.length; o++)l += "<option value='" + k(m.keys[o]) + "'><![CDATA[" + m.values[o] + "]]\></option>";
        l += "</column>"
    }
    return l += "</head>"
};
if (window.eXcell_sub_row_grid)window.eXcell_sub_row_grid.prototype.getContent = function () {
    return""
};
dhtmlXGridObject.prototype.toExcel = function (q, k, l, a, g) {
