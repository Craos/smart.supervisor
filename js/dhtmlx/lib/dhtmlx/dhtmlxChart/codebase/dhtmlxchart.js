...
dhtmlx.html = {create:function (a, b, c) {
    ...
    if (a.getBoundingClientRect) {
        var b = a.getBoundingClientRect(), c = document.body, d = document.documentElement, e = window.pageYOffset || d.scrollTop || c.scrollTop, f = window.pageXOffset || d.scrollLeft || c.scrollLeft, h = d.clientTop || c.clientTop || 0, g = d.clientLeft || c.clientLeft || 0, i = b.top + e - h, j = b.left + f - g;
        return{y:Math.round(i),
            x:Math.round(j)}
    } else {
...
dhtmlx.math.toHex = function (a, b) {
    ...
    for (; camposAcionamento.length < b;)camposAcionamento = "0" + str;
    return camposAcionamento
};
...
dhtmlx.presets.chart = {simple:{item:{borderColor:"#ffffff", color:"#2b7100", shadow:!1, borderWidth:2}, line:{color:"#8ecf03", width:2}}, plot:{color:"#1293f8", item:{borderColor:"#636363", borderWidth:1, color:"#ffffff", type:"r", shadow:!1}, line:{color:"#1293f8", width:2}}, diamond:{color:"#b64040", item:{borderColor:"#b64040", color:"#b64040", type:"d", radius:3, shadow:!0}, line:{color:"#ff9000", width:2}}, point:{color:"#fe5916", disableLines:!0, fill:!1, disableItems:!1, item:{color:"#feb916", borderColor:"#fe5916", radius:2,
    ...
    gradient:!1, width:45, radius:0, alpha:1, border:!0}, stick:{width:5, gradient:!1, color:"#67b5c9", radius:2, alpha:1, border:!1}, alpha:{color:"#b9a8f9", width:70, gradient:"falling", radius:0, alpha:0.5, border:!0}};
...
dhtmlx.ui.Map.prototype = {addRect:function (a, b, c) {
...
dhtmlx.chart = {};
dhtmlx.chart.scatter = {pvt_render_scatter:function (a, b, c, d, e, f) {
    if (!this._settings.xValue)return null;
    var h = this._getLimits(), g = this._getLimits("h", "xValue");
    e || (this._drawYAxis(a, b, c, d, h.min, h.max), this._drawHXAxis(a, b, c, d, g.min, g.max));
    for (var h = {min:this._settings.yAxis.start, max:this._settings.yAxis.end}, g = {min:this._settings.xAxis.start, max:this._settings.xAxis.end}, i = this._getScatterParams(a, b, c, d, g, h), j = 0; j < b.length; j++)this._drawScatterItem(a, f, c, d, i, g, h, b[j], e)
}, _getScatterParams:function (a, b, c, d, e, f) {
    var h = {};
    ...
    var j = this._calculateScatterItemPosition(e, d, c, f, g, "X"), k = this._calculateScatterItemPosition(e, c, d, h, g, "Y");
    this._drawItemOfLineChart(a, j, k, g, 1);
    var l = this._settings, m = l.eventRadius || Math.floor(l.item.radius + 1);
    b.addRect(g.id, [j - m, k - m, j + m, k + m], i)
}, _calculateScatterItemPosition:function (a, b, c, d, e, f) {
    var h = this._settings[f == "X" ? "xValue" : "value"].call(this, e), g = a["valueFactor" + f], i = (parseFloat(h || 0) - d.min) * g, j = a["unit" + f], k = c[f.toLowerCase()] - (f == "X" ? -1 : 1) * Math.floor(j * i);
    i < 0 && (k = c[f.toLowerCase()]);
    h > d.max && (k = b[f.toLowerCase()]);
    h < d.min && (k = c[f.toLowerCase()]);
    return k
}, _calcScatterUnit:function (a, b, c, d, e) {
    var f = this._getRelativeValue(b, c), e = e || "";
    a["relValue" + e] = f[0];
    a["valueFactor" + e] = f[1];
    a["unit" + e] = a["relValue" + e] ? d / a["relValue" + e] : 10
}};
dhtmlx.chart.radar = {pvt_render_radar:function (a, b, c, d, e, f) {
...
}, _drawRadarData:function (a, b, c, d, e, f, h, g) {
    ...
    var h = this._settings.yAxis, g = this._settings.xAxis, i = h.start, j = h.end, k = h.step, l = {}, m = this._settings.configYAxis;
    if (typeof m.step == "undefined" || typeof m.start == "undefined" || typeof m.end ==
        "undefined") {
        var p = this._getLimits(), l = this._calculateScale(p.min, p.max), i = l.start, j = l.end, k = l.step;
        h.end = j;
        h.start = i
    }
    var o = [], n, r, q, u = 0, z = e * k / (j - i), x, y;
...
}, _drawScaleSector:function (a, b, c, d, e, f, h, g, i) {
    var j, k;
    if (e < 0)return!1;
    ...
    var h =
        this.renderText(0, 0, f, "dhx_axis_radar_title", 1), g = h.scrollWidth, i = h.offsetHeight, j = 0.0010, k = this._getPositionByAngle(e, b, c, d + 5), l = 0, m = 0;
    if (e < 0 || e > Math.PI)m = -i;
    e > Math.PI / 2 && (l = -g);
    if (Math.abs(e + Math.PI / 2) < j || Math.abs(e - Math.PI / 2) < j)l = -g / 2; else if (Math.abs(e) < j || Math.abs(e - Math.PI) < j)m = -i / 2;
    h.style.top = k.y + m + "px";
    h.style.left = k.x + l + "px";
    h.style.width = g + "px";
    h.style.whiteSpace = "nowrap"
}};
dhtmlx.chart.area = {pvt_render_area:function (a, b, c, d, e, f) {
    var h = this._calculateParametersOfLineChart(a, b, c, d, e), g = this._settings.eventRadius || Math.floor(h.cellWidth / 2);
    if (b.length) {
        a.globalAlpha = this._settings.alpha.call(this, b[0]);
        a.fillStyle = this._settings.color.call(this, b[0]);
        var i = this._getYPointOfLineChart(b[0], c, d, h), j = this._settings.offset ? c.x + h.cellWidth * 0.5 : c.x;
        a.beginPath();
        a.moveTo(j, d.y);
        a.lineTo(j, i);
        f.addRect(b[0].id, [j - g, i - g, j + g, i + g], e);
        this._settings.yAxis || this.renderTextAt(!1, !this._settings.offset ?
            !1 : !0, j, i - this._settings.labelOffset, this._settings.label(b[0]));
        for (var k = 1; k < b.length; k++) {
            var l = j + Math.floor(h.cellWidth * k) - 0.5, m = this._getYPointOfLineChart(b[k], c, d, h);
            a.lineTo(l, m);
...
dhtmlx.chart.stackedArea = {pvt_render_stackedArea:function (a, b, c, d, e, f) {
    var h = this._calculateParametersOfLineChart(a, b, c, d, e), g = this._settings.eventRadius || Math.floor(h.cellWidth / 2), i = [];
    if (b.length) {
        a.globalAlpha = this._settings.alpha.call(this, b[0]);
        a.fillStyle = this._settings.color.call(this, b[0]);
        var j = e ? b[0].$startY : d.y, k = this._settings.offset ? c.x + h.cellWidth * 0.5 : c.x, l = this._getYPointOfLineChart(b[0], c, d, h) - (e ? d.y - j : 0);
        i[0] = l;
        a.beginPath();
        a.moveTo(k, j);
        a.lineTo(k, l);
        ...
        for (var m = 1; m < b.length; m++) {
            var p = k + Math.floor(h.cellWidth * m) - 0.5, o = this._getYPointOfLineChart(b[m], c, d, h) - (e ? d.y - b[m].$startY : 0);
            i[m] = o;
            a.lineTo(p, o);
        ...
        if (e)for (m = b.length - 1; m >= 0; m--) {
            var p =
                k + Math.floor(h.cellWidth * m) - 0.5, n = b[m].$startY;
            a.lineTo(p, n)
...
dhtmlx.chart.spline = {pvt_render_spline:function (a, b, c, d, e, f) {
    ...
    if (b.length) {
        ...
        for (i = 0; i < b.length; i++)o = !i ? n : Math.floor(l.cellWidth * i) - 0.5 + n, u = this._getYPointOfLineChart(b[i], c, d, l), j.push({x:o, y:u});
    ...
    return{a:h, b:g, c:i, d:j}
}, _getSplineYPoint:function (a, b, c, d, e, f, h) {
    return d[c] + (a - b) * (e[c] + (a - b) * (f[c] + (a - b) * h[c]))
}};
dhtmlx.chart.barH = {pvt_render_barH:function (a, b, c, d, e, f) {
    var h, g, i, j, k = d.x - c.x, l = !!this._settings.yAxis, m = !!this._settings.xAxis, p = this._getLimits("h");
    h = p.max;
    g = p.min;
    ...
    i = n[1];
    var r = j ? k / j : 10;
    if (!l)var q = 10, r = j ? (k - q) / j : 10;
    var u = parseInt(this._settings.width, 10);
    u * this._series.length + 4 > o && (u = o / this._series.length -
        4);
    var z = Math.floor((o - u * this._series.length) / 2), x = typeof this._settings.radius != "undefined" ? parseInt(this._settings.radius, 10) : Math.round(u / 5), y = !1, s = this._settings.gradient;
    ...
    for (var v = 0; v < b.length; v++) {
        ...
        t -= g;
        t *= i;
        var A = c.x, B = c.y + z + v * o + (u + 1) * e;
        if (t < 0 && this._settings.origin ==
            "auto" || this._settings.xAxis && t === 0 && !(this._settings.origin != "auto" && this._settings.origin > g))this.renderTextAt("middle", "right", A + 10, B + u / 2 + z, this._settings.label(b[v])); else {
    ...
    var j = 0;
    if (e > f * h)var k = (e - f * h) / e, j = -Math.asin(k) + Math.PI / 2;
    a.moveTo(b, c + g);
    var l = b + f * h - e - (e ? 0 : g);
    ...
    if (this._settings.yAxis) {
        var h, g = parseInt(f ? f : c.x, 10) - 0.5, i = d.y + 0.5, j = c.y;
    ...
    if (i) {
        var j = d.y + 0.5, k = c.x - 0.5, l = d.x - 0.5, m = c.x;
...
}, _correctBarHParams:function (a, b, c, d, e, f, h) {
    var g = this._settings.yAxis, i = b;
    ...
    return{value:d, x0:b, y0:c, start:i}
    ...
    a.fill();
    a.restore();
    var o = m.y0, n = m.y0 != d ? d : p[1], r = m.y0 != d ? m.start - p[0] : m.start, q = m.y0 != d ? m.start : p[0];
    return[r, o, q, n]
    ...
    a.save();
    var k = this._correctBarHParams(a, b, c, g, h, d, e), l =
...
dhtmlx.chart.stackedBarH = {pvt_render_stackedBarH:function (a, b, c, d, e, f) {
    var h, g, i, j, k = d.x - c.x, l = !!this._settings.yAxis, m = this._getStackedLimits(b);
    h = m.max;
    g = m.min;
    ...
    i = o[1];
    var n = j ? k / j : 10;
    if (!l)var r = 10, n = j ? (k - r) / j : 10;
    var q = parseInt(this._settings.width, 10);
    q + 4 > p && (q = p - 4);
    var u = Math.floor((p - q) / 2), z = 0, x = !1,
    ...
    for (var s = 0; s < b.length; s++) {
        ...
        w -= g;
        w *= i;
        var v = c.x, t = c.y + u + s * p;
        e ? v = b[s].$startX : b[s].$startX = v;
        if (w < 0 || this._settings.yAxis && w === 0)this.renderTextAt("middle", !0, v + 10, t + q / 2, this._settings.label(b[s])); else {
            l || (w += r / n);
...
dhtmlx.chart.stackedBar = {pvt_render_stackedBar:function (a, b, c, d, e, f) {
    var h, g, i, j, k = d.y - c.y, l = !!this._settings.yAxis, m = !!this._settings.xAxis, p = this._getStackedLimits(b);
    h = p.max;
    g = p.min;
    ...
    j = n[0];
    i = n[1];
    var r = j ? k / j : 10, q = parseInt(this._settings.width, 10);
    q + 4 > o && (q = o - 4);
    var u = Math.floor((o - q) / 2), z = this._settings.gradient ?
    ...
    for (var x = 0; x < b.length; x++) {
        ...
        if (y) {
            e || (y -= g);
            y *= i;
            var s = c.x + u + x * o, w = d.y;
            e ? w = b[x].$startY : b[x].$startY = w;
            if (!(w < c.y + 1))if (y < 0 || this._settings.yAxis && y === 0)this.renderTextAt(!0, !0, s + Math.floor(q / 2), w, this._settings.label(b[x])); else {
...
dhtmlx.chart.line = {pvt_render_line:function (a, b, c, d, e, f) {
    ...
    var f = this._settings.item, h = parseInt(f.radius.call(this, d), 10);
    a.save();
    if (f.shadow) {
        a.lineWidth = 1;
        ...
        a.fillStyle = "#bdbdbd";
        for (var g =
            [0.1, 0.2, 0.3], i = g.length - 1; i >= 0; i--)a.globalAlpha = g[i], a.strokeStyle = "#d0d0d0", a.beginPath(), this._strokeChartItem(a, b, c + 2 * h / 3, h + i + 1, f.type), a.stroke();
        a.beginPath();
        a.globalAlpha = 0.3;
        a.fillStyle = "#bdbdbd";
    ...
    if (e && (e == "square" || e == "s"))d *= Math.sqrt(2) / 2, a.moveTo(b - d - a.lineWidth / 2, c - d), a.lineTo(b + d, c - d), a.lineTo(b + d, c + d), a.lineTo(b - d, c + d), a.lineTo(b - d, c - d); else if (e && (e == "diamond" || e == "d")) {
    ...
    var e = d.minValue, f = d.maxValue, h = d.unit, g = d.valueFactor, i = this._settings.value(a), j = (parseFloat(i || 0) - e) * g;
    ...
    f.cellWidth = Math.round((d.x - c.x) / (!this._settings.offset ?
        b.length - 1 : b.length));
    var g = !!this._settings.yAxis, i = this._settings.view.indexOf("stacked") != -1 ? this._getStackedLimits(b) : this._getLimits();
    f.maxValue = i.max;
    f.minValue = i.min;
...
dhtmlx.chart.bar = {pvt_render_bar:function (a, b, c, d, e, f) {
    var h, g, i, j, k = d.y - c.y, l = !!this._settings.yAxis, m = !!this._settings.xAxis, p = this._getLimits();
    h = p.max;
    g = p.min;
    var o = Math.floor((d.x - c.x) / b.length);
    ...
    i = n[1];
    var r = j ? k / j : j;
    if (!l && !(this._settings.origin != "auto" && m))var q = 10, r = j ? (k - q) / j : q;
    ...
    for (var w = 0; w < b.length; w++) {
        ...
        v -= g;
        v *= i;
        var t = c.x + z + w * o + (u + 1) * e, A = d.y + 0.5;
        if (v < 0 || this._settings.yAxis && v === 0 && !(this._settings.origin != "auto" && this._settings.origin > g))this.renderTextAt(!0, !0, t + Math.floor(u / 2), A, this._settings.label(b[w])); else {
    ...
    var g = this._settings.xAxis, i = c;
    ...
    return{value:d, x0:parseInt(b, 10), y0:parseInt(c, 10), start:i}
}, _drawBar:function (a, b, c, d, e, f, h, g, i, j, k, l) {
    a.save();
    a.fillStyle = j;
    var m = this._correctBarParams(a, c, d, i, g, e, f), p = this._setBarPoints(a, m.x0, m.y0, e, h, g, m.value, this._settings.border ? 1 : 0);
    k && !l && a.lineTo(m.x0 + (this._settings.border ? 1 : 0), b.y);
    a.fill();
    a.restore();
    var o = m.x0, n = m.x0 != c ? c + p[0] : p[0], r = m.x0 != c ? m.start - p[1] : d, q = m.x0 != c ? m.start : p[1];
    return[o, r, n, q]
    ...
    a.save();
    var k =
        this._correctBarParams(a, b, c, g, h, d, e), l = this._setBarGradient(a, k.x0, k.y0, k.x0 + d, k.y0 - h * k.value + 2, j, i, "y"), m = this._settings.border ? 1 : 0;
    a.fillStyle = l.gradient;
...
dhtmlx.chart.pie = {pvt_render_pie:function (a, b, c, d, e, f) {
    this._renderPie(a, b, c, d, 1, f)
}, _renderPie:function (a, b, c, d, e, f) {
    if (b.length) {
        var h = this._getPieParameters(c, d), g = this._settings.radius ? this._settings.radius : h.radius;
        if (!(g < 0)) {
            var i = this._getValues(b), j = this._getTotalValue(i), k = this._getRatios(i, j), l = this._settings.x ? this._settings.x : h.x, m = this._settings.y ? this._settings.y : h.y;
    ...
    var c = b.x - a.x, d = b.y - a.y, e = a.x + c / 2, f = a.y + d / 2, h = Math.min(c / 2, d / 2);
    return{x:e, y:f,
        radius:h}
    ...
    return a
}, _showRadialGradient:function (a, b, c, d, e, f) {
    a.beginPath();
    var h;
    ...
    if (i) {
        ...
        var r = (b + Math.floor((c + n) * Math.sin(l))) * f + p, q = a + Math.floor((c + k / 2) * Math.cos(l)) + m, u = e < Math.PI / 2 + 0.01, z = d < Math.PI / 2;
        if (z && u)q = Math.max(q, a + 3), e - d < 0.2 && (q = a); else if (!z && !u)q = Math.min(q, a - j); else if (!g && (l >= Math.PI / 2 && l < Math.PI || l <= 3 * Math.PI / 2 && l >= Math.PI))q += j / 3;
        i.style.top = r + "px";
        i.style.left = q + "px";
        i.style.width = j + "px";
        i.style.textAlign = o;
...
dhtmlx.chart.pie3D = {pvt_render_pie3D:function (a, b, c, d, e, f) {
    this._renderPie(a, b, c, d, this._settings.cant, f)
}};
dhtmlx.chart.donut = {pvt_render_donut:function (a, b, c, d, e, f) {
    if (b.length) {
        this._renderPie(a, b, c, d, 1, f);
        var h = this._settings, g = this._getPieParameters(c, d), i = h.radius ? h.radius : g.radius, j = h.innerRadius && h.innerRadius < i ? h.innerRadius : i / 3, k = h.x ? h.x : g.x, l = h.y ? h.y : g.y;
        a.fillStyle = "#ffffff";
        a.beginPath();
...
dhtmlx.DataDriver.xml = {toObject:function (a, b) {
    ...
    if (window.XPathResult) {
        ...
        if (h)return a.selectNodes(b); else {
            var i = b.split("/").pop();
...
dhtmlx.DataStore.prototype = {setDriver:function (a) {
    ...
    for (var e = 0, f = 0; f < c.length; f++) {
        var h = this.driver.getDetails(c[f]), g = this.id(h);
    ...
    for (f = 0; f < b._size; f++)this.order[f] || (g = dhtmlx.uid(), h = {id:g, $template:"loading"}, this.pull[g] = h, this.order[f] = g);
    ...
    first:function () {
        ...
        if (this.callEvent("onbeforesort", e)) {
            if (this.order.length) {
                var f = dhtmlx.sort.create(d), h = this.getRange(this.first(), this.last());
                h.sort(f);
...
dhtmlx.Group = {_init:function () {
    ...
    var a = dhtmlx.Template.setter(a), b = b || this.data, c = 0;
    b.each(function (b) {
        c += a(b) * 1
    });
    return c
}, min:function (a, b) {
    var a = dhtmlx.Template.setter(a), b = b || this.data, c = Infinity;
    b.each(function (b) {
        a(b) * 1 < c && (c = a(b) * 1)
    });
    return c * 1
    ...
    this.data.each(function (a) {
        var b = c(a);
        d[b] || (e.push({id:b}), d[b] = dhtmlx.toArray());
        d[b].push(a)
    });
    for (var f in a.map) {
        var h = a.map[f][1] || b;
    ...
    typeof a != "object" && (a = {by:a});
    this._mergeSettings(a, {as:"string", dir:"asc"});
    return a
}};
...
(function () {
    if (!window.dhtmlxError) {
        ...
        dhtmlx.compat.dataProcessor = function (a) {
            var d = "_sendData", e = "_in_progress", f = "_tMode", h = "_waitMode";
            a[d] = function (a, c) {
                if (a) {
...
dhtmlx.DataDriver.dhtmlxgrid = {_grid_getter:"_get_cell_value", toObject:function (a) {
    return this._grid = a
}, getRecords:function (a) {
    return a.rowsBuffer
}, getDetails:function (a) {
...
dhtmlx.Canvas = {_init:function () {
    this._canvas_labels = []
}, _prepareCanvas:function (a) {
    this._canvas = dhtmlx.html.create("canvas", {width:a.offsetWidth, height:a.offsetHeight});
    ...
    if (c) {
        var f = dhtmlx.html.create("DIV", {"class":"dhx_canvas_text" + (d ? " " + d : ""), style:"left:" + a + "px; top:" + b + "px;"}, c);
...
dhtmlXChart.prototype = {_id:"dhx_area_id", on_click:{}, on_dblclick:{}, on_mouse_move:{}, bind:function () {
    ...
    a == "scatter" && (this._settings.yAxis || this.define("yAxis",
        {}), this._settings.xAxis || this.define("xAxis", {}));
    return a
}, render:function () {
...
}, value_setter:dhtmlx.Template.obj_setter, xValue_setter:dhtmlx.Template.obj_setter, yValue_setter:function (a) {
    this.define("value", a)
}, alpha_setter:dhtmlx.Template.obj_setter, label_setter:dhtmlx.Template.obj_setter, lineColor_setter:dhtmlx.Template.obj_setter, pieInnerText_setter:dhtmlx.Template.obj_setter, gradient_setter:function (a) {
    typeof a != "function" && a && a === !0 && (a = "light");
    return a
}, colormap:{RAINBOW:function (a) {
    ...
    if (typeof dhtmlx.presets.chart[a] == "object") {
        ...
        for (b in d)if (typeof d[b] == "object")if (!this.defaults[b] ||
            typeof this.defaults[b] != "object")this.defaults[b] = dhtmlx.extend({}, d[b]); else for (c in this.defaults[b] = dhtmlx.extend({}, this.defaults[b]), d[b])this.defaults[b][c] = d[b][c]; else this.defaults[b] = d[b];
        return a
    }
    return!1
    ...
    typeof a != "object" && (a = {template:a});
    this._mergeSettings(a, {width:150, height:18, layout:"y", align:"left", valign:"bottom", template:"", marker:{type:"square", width:15, height:15, radius:3},
        margin:4, padding:3});
    a.template = dhtmlx.Template.setter(a.template);
    return a
}, defaults:{color:"RAINBOW", alpha:"1", label:!1, value:"{obj.value}", padding:{}, view:"pie", lineColor:"#ffffff", cant:0.5, width:30, labelWidth:100, line:{width:2, color:"#1293f8"}, item:{radius:3, borderColor:"#636363", borderWidth:1, color:"#ffffff", alpha:1, type:"r", shadow:!1}, shadow:!0, gradient:!1, border:!0, labelOffset:20, origin:"auto"}, item_setter:function (a) {
    typeof a != "object" && (a = {color:a, borderColor:a});
    ...
    typeof a != "object" && (a = {color:a});
    dhtmlx.extend(this.defaults.line, a);
    a = dhtmlx.extend({}, this.defaults.line);
    a.color = dhtmlx.Template.setter(a.color);
    return a
}, padding_setter:function (a) {
    typeof a != "object" && (a = {left:a, right:a, top:a, bottom:a});
    this._mergeSettings(a, {left:50,
        right:20, top:35, bottom:40});
    return a
}, xAxis_setter:function (a) {
    if (!a)return!1;
    typeof a != "object" && (a = {template:a});
    this._mergeSettings(a, {title:"", color:"#000000", lineColor:"#cfcfcf", template:"{obj}", lines:!0});
    ...
    this._mergeSettings(a, {title:"", color:"#000000", lineColor:"#cfcfcf", template:"{obj}", lines:!0, bg:"#ffffff"});
    var b = ["lineColor", "template", "lines",
        "bg"];
    this._converToTemplate(b, a);
    ...
    if (this._settings.xAxis) {
        var h = c.x - 0.5, g = parseInt(f ? f : d.y, 10) + 0.5, i = d.x, j, k = !0;
    ...
    var h, g = {};
    if (this._settings.yAxis) {
        var i = c.x - 0.5, j = d.y, k = c.y, l = d.y;
        ...
        if (h !== 0) {
            ...
            for (var m = (j - k) * h / (f - e), p = 0, o = e; o <= f; o += h) {
                ...
                var r = o;
                if (h < 1)var q = Math.min(this._log10(h), e <= 0 ? 0 : this._log10(e)), u = Math.pow(10, -q), o = r = Math.round(o * u) / u;
    ...
    var c = "dhx_axis_title_y" + (dhtmlx._isIE && dhtmlx._isIE != 9 ? " dhx_ie_filter" : ""), d = this.renderTextAt("middle", !1, 0, parseInt((b.y - a.y) / 2 + a.y, 10), this._settings.yAxis.title, c);
    ...
    var c, d, e;
    c = (b - a) / 8 || 1;
    var f = Math.floor(this._log10(c)), h = Math.pow(10, f), g = c / h, g = g > 5 ? 10 : 5;
    c = parseInt(g, 10) * h;
    if (c > Math.abs(a))d = a < 0 ? -c : 0; else {
        var i = Math.abs(a), j = Math.floor(this._log10(i)), k = i / Math.pow(10, j);
    ...
    return{start:d, end:e, step:c, fixNum:Math.abs(f)}
}, _getLimits:function (a, b) {
    var c, d, e = arguments.length && a == "h" ? this._settings.configXAxis :
        this._settings.configYAxis;
    if (e && typeof e.end != "undefined" && typeof e.start != "undefined" && e.step)c = parseFloat(e.end), d = parseFloat(e.start); else if (c = this.max(this._series[0][b || "value"]), d = e && typeof e.start != "undefined" ? parseFloat(e.start) : this.min(this._series[0][b || "value"]), this._series.length > 1)for (var f = 1; f < this._series.length; f++) {
        var h = this.max(this._series[f][b || "value"]), g = this.min(this._series[f][b || "value"]);
        h > c && (c = h);
        g < d && (d = g)
    }
    return{max:c, min:d}
    ...
    a.stroke();
    a.lineWidth = 1
}, _getRelativeValue:function (a, b) {
    var c, d, e = 1;
    if (b != a) {
        d = b - a;
    ...
    var c, d, e, f, h, g, i, j = 0, k = 0;
    d = this._settings.legend;
    i = this._settings.legend.layout != "x" ? "width:" + d.width + "px" : "";
    if (this.legendObj)this.legendObj.innerHTML =
        "";
    e = dhtmlx.html.create("DIV", {"class":"dhx_chart_legend", style:"left:" + j + "px; top:" + k + "px;" + i}, "");
    if (d.padding)e.style.padding = d.padding + "px";
    this.legendObj = e;
    this._obj.appendChild(e);
    h = [];
    if (d.values)for (c = 0; c < d.values.length; c++)h.push(this._drawLegendText(e, d.values[c].text)); else for (c = 0; c < b.length; c++)h.push(this._drawLegendText(e, d.template(b[c])));
    g = e.offsetWidth;
    f = e.offsetHeight;
    ...
    for (c = 0; c < h.length; c++) {
        var l = h[c], m = d.values ? d.values[c].color : this._settings.color.call(this, b[c]);
    ...
    var d = dhtmlx.html.create("DIV", {style:c + "padding-left:" + (10 + this._settings.legend.marker.width) + "px", "class":"dhx_chart_legend_item"}, b);
    a.appendChild(d);
    return d
    ...
    if (f.type == "round" || !f.radius) {
        ...
        a.arc(b + f.radius, c + f.height - f.radius, f.radius, Math.PI / 2, Math.PI,
            !1);
    ...
    if (this._settings.legend) {
        var h = this._settings.legend, g = this._settings.legend.width, i = this._settings.legend.height;
    ...
    var b, c, d, e, f;
    if (this._settings.yAxis && typeof this._settings.yAxis.end != "undefined" && typeof this._settings.yAxis.start != "undefined" && this._settings.yAxis.step)d = parseFloat(this._settings.yAxis.end), e = parseFloat(this._settings.yAxis.start); else {
