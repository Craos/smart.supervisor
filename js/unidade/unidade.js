...
function unidade_cadastro() {
    ...
    formUnidade = tabbarUnidade.cells('tabInformacoes').attachForm(campos_info_unidade);

    var tabnotificacoes = tabbarUnidade.cells('tabNotificacoes');

    sessionStorage.recursocorrente = 'unidade_cadastro';
    ...
    formnotificacoes = cell_a.attachForm(campos_notificacoes);
...
function CarregaInformacoes(CarregaNotificacoes) {
    ...
    admunidade.Listar({
        filter: {
            num: JSON.parse(sessionStorage.unidadecorrente).pk_unidade
        },
        callback: function (response) {

            sys.FormClear(objformUnidade);

            let dados = response.dados[0];
            objformUnidade.setFormData(dados);
        ...
        list: [
            ...
            },
            {
                type: "block", offsetTop: "0", list: [
                    {
                        type: "input",
                        name: "email_correspondencias",
                        label: "E-mail para setor de correspond&ecirc;ncias 1",
                        inputWidth: 350,
                        style: "font-weight:bold;"
                    },
                    {type: "newcolumn"},
                    {
                        type: "input",
                        name: "email_correspondencias2",
                        label: "E-mail para setor de correspond&ecirc;ncias 2",
                        inputWidth: 350,
                        style: "font-weight:bold;"
                    }
                ]
