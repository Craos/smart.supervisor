...
dhtmlXGridObject.prototype._filterTreeA = function (column, value) {
    ...
    var add_line = function (el, s, t) {
        ...
        if (!t) {
            t = {id:el.id, childs:[], level:el.level, parent:z, index:z.childs.length, image:el.image, state:el.state, buff:el.buff, has_kids:el.has_kids};
            z.childs.push(t);
            temp.get[t.id] = t
        }
        ;
    ...
    var temp_c = function (el) {
        for (var i = 0; i < el.childs.length; i++)that.temp(el.childs[i])
    };
    switch (mode.toString()) {
        case "-2":
            check = function (el) {
                if (fbf[el.id])return false;
                temp_c(el);
                return true
            };
            break;
        case "-1":
            check = function (el) {
                return !el.childs.length
            };
            break;
        default:
            check = function (el) {
                return mode == el.level
            };
            break
    }
    ;
    this.temp = function (el) {
        if (el.id != 0 && check(el)) {
            if (d ? (this._get_cell_value(el.buff, column).toString().toLowerCase().indexOf(value) == -1) : (!value(this._get_cell_value(el.buff, column), el.id))) {
                ...
                if (el.childs && mode != -2)this._h2.forEachChild(el.id, function (cel) {
    ...
    temp.forEachChild(0, function (el) {
        if (!el.childs.length && !el.has_kids) {
            ...
            if (el.buff.tagName == "TR") {
                ...
                if (prev && prev.state != "blank")el.state = prev.state;
                el.update = true;
                this._updateTGRState(el)
            }
        }
