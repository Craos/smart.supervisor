...
dhtmlXGridObject.prototype.enableTreeGridLines = function () {
    ...
    this.attachEvent("onRowAdded", function (id) {
        ...
        if (z.index < (z.parent.childs.length - 1)) {
            ...
            this._updateLine(z);
            this._updateParentLine(z);
            if (z.childs.length)this._h2.forEachChild(z.id, function (c_el) {
    ...
    this.attachEvent("onOpen", function (id, state) {
        if (state) {
            var z = this._h2.get[id];
            for (var i = 0; i < z.childs.length; i++)this._updateParentLine(z.childs[i])
        }
        ;
        return true
    });
    this.attachEvent("onBeforeRowDeleted", function (id) {
        ...
        var z = this._h2.get[id];
        var w = null;
        if (z.index != 0)w = z.parent.childs[z.index - 1];
        z = z.parent;
        window.setTimeout(function () {
            ...
            if (w) {
                self._updateLine(w);
                if (w.state == "minus")self._h2.forEachChild(w.id, function (z) {
                    self._updateParentLine(z)
                }, self)
            }
        }, 1);
