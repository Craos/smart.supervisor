...
$instrucao_fila = /** @lang PostgreSQL */
    <<<TAG
    SELECT bloco, admunidade, to_char(enviado, 'DD/MM/YYYY HH24:MI:ss') AS data, emails.mensagem, LPAD(correspondencias.num::TEXT, 7, '0')::VARCHAR AS num, correspondencias.num AS sequencia
...
while ($dados = pg_fetch_object($lista_unidades)) {
    ...
    $instrucao_enderecos = <<<EMAILS
        SELECT email AS endereco
         ...
         WHERE bloco = $dados->bloco AND admunidade = $dados->unidade
         GROUP BY email
EMAILS;
