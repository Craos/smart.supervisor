...
SystemCraos = function () {
    ...
    this.FormAction = function (Params, CallBack) {
        ...
        //sys.CallbackServer.setRequestHeader('Content-length', Params.length);
        //sys.CallbackServer.setRequestHeader('Connection', 'close');
    ...
    this.obtemDataEntradaFormatada = function (filedate) {
        ...
        var dataAtual = new Date(filedate);
    ...
    this.FormLoad = function (Form, http, foto) {
        ...
        if (itens.foto1 != undefined && foto != undefined) {
            var fotocadastro = Form.getContainer(foto);
    ...
    this.salvaRegistro = function () {
        ...
        var today = new Date();
        var lastdate = today.format("yyyy-mm-dd");
        var lasttime = today.format("HH:MM:ss");
        var originalUserInfo = JSON.parse(sessionStorage.originalUserInfo);
        var userinfo = JSON.parse(sessionStorage.userinfo);
    ...
    this.trataRetornodoServidor = function (http) {
        ...
        var matrixinformacoesservidor;
        matrixinformacoesservidor = JSON.parse(http.responseText);

        var mensagemalerta = 'Houve um erro ao enviar suas informações. Por favor tente mais tarde!';
        ...
        if (sys.PostServer != undefined)
            for (var i = 0; i < sys.PostServer.length; i++)
    ...
    this.formtoWindow = function (Titulo, Largura, Altura, Estrutura, Informacoes, foto, OcultarItens) {
        ...
        var windowID = 'windowView';
        var windowsformtoWindow;
        ...
        var formview = windowView.attachForm(Estrutura);

        this.FormLoad(formview, Informacoes, foto);

        if (OcultarItens != undefined)
            for (var i = 0; i < OcultarItens.length; i++) {
