...
dhtmlxValidation.prototype = {trackInput:function (a, b, c, d) {
    dhtmlxEvent(a, "keyup", function () {
        if (dhtmlxValidation._timer)window.clearTimeout(dhtmlxValidation._timer), dhtmlxValidation._timer = null;
        dhtmlxValidation._timer = window.setTimeout(function () {
            if (dhtmlxValidation.checkInput(a, b))a.className = a.className.replace(/[ ]*dhtmlx_live_validation_error/g, ""), d && d(a, a.value, b); else if (!c || c(a, a.value, b))a.className += " dhtmlx_live_validation_error"
        }, 250)
    })
}, checkInput:function (a, b) {
    return this.checkValue(a.value,
        b)
...
dhtmlXGridObject.prototype.enableValidation = function (a, b) {
    this._validators = (a = convertStringToBoolean(a)) ? {data:[]} : !1;
    if (arguments.length > 1)this._validators._live = b;
...
dhtmlXGridObject.prototype.setColValidators = function (a) {
    this._validators || this.enableValidation(!0);
    typeof a == "string" && (a = a.split(this.delim));
    this._validators.data = a
};
dhtmlXGridObject.prototype.validationEvent = function (a, b, c, d) {
    var f = this._validators;
    if (!f)return!0;
    var g = f.data[c] || this.cells(b, c).getAttribute("validate") || "";
    if (a == 1 && g) {
        ...
        if (f._live) {
            var h = this;
            dhtmlxValidation.trackInput(e.getInput(), g, function (a, d, e) {
                return h.callEvent("onLiveValidationError", [b, c, d, a, e])
            }, function (a, d, e) {
                return h.callEvent("onLiveValidationCorrect",
                    [b, c, d, a, e])
            })
        }
    }
    a == 2 && this.validateCell(b, c, g, d);
    return!0
};
dhtmlXGridObject.prototype.validateCell = function (a, b, c, d) {
    c = c || this._validators.data[b] || this.cells(a, b).getAttribute("validate");
    d = d || this.cells(a, b).getValue();
    if (c) {
        var f = this.cells(a, b).cell, g = !0;
        typeof c == "string" && (c = c.split(this.delim));
        for (var e = 0; e < c.length; e++)dhtmlxValidation.checkValue(d, c[e]) || (this.callEvent("onValidationError", [a, b, d, c[e]]) && (f.className += " dhtmlx_validation_error"), g = !1);
        if (g)this.callEvent("onValidationCorrect", [a, b, d, c]), f.className = f.className.replace(/[ ]*dhtmlx_validation_error/g,
            "");
        return g
    }
};
