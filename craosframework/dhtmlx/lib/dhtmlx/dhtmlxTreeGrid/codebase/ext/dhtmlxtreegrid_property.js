...
function eXcell_list(cell) {
    ...
    this.edit = function () {
        ...
        for (var i = 0; i < opt.length; i++)this.obj.options[i] = new Option(opt[i], opt[i]);
...
function dhtmlXPropertyGrid(cont) {
    ...
    t.attachEvent("onEditCell", function (stage, id, ind, nv, ov) {
        if (stage == 1 && this.editor && this.editor.obj && this.editor.obj.select)this.editor.obj.select();
        if (stage == 2 && ov != nv) {
    ...
    t.setProperties = function (data) {
        ...
        this.forEachRow(function (id) {
            ...
            if (typeof data[t] != "undefined")this.cells(id, 1).setValue(data[t])
        })
