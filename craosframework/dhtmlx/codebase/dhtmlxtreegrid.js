...
dhtmlXGridObject.prototype._updateTGRState = function (z) {
    ...
    if (this.rowsAr[z.id].imgTag)this.rowsAr[z.id].imgTag.src = this.imgURL + z.state + ".gif";
    z.update = false
};
dhtmlXGridObject.prototype.doExpand = function (obj) {
    ...
    else if ((r.state == "minus") && (!r._closeable))
...
function dhtmlxHierarchy() {
    ...
    this.addAfter = function (id, parentId, afterId, fix) {
        ...
        if (afterId)var ind = this.get[afterId].index + (fix ? 0 : 1); else var ind = z.childs.length;
    ...
    this.remove = function (id) {
        var z = this.get[id || 0];
        for (var i = 0; i < z.childs.length; i++)this.deleteAll(z.childs[i].id)
        z.childs = [];
        z.parent.childs = z.parent.childs.slice(0, z.index).concat(z.parent.childs.slice(z.index + 1));
        for (var i = z.index; i < z.parent.childs.length; i++)z.parent.childs[i].index--;
        delete this.get[id]
    };
...
dhtmlXGridObject.prototype._getOpenLenght = function (id, start) {
    ...
    start += z.length;
    for (var i = 0; i < z.length; i++)if (z[i].childs.length && z[i].state == 'minus')start += this._getOpenLenght(z[i].id, 0);
    return start
};
dhtmlXGridObject.prototype.collapseKids = function (curRow) {
    ...
    if (this._srnd || this.pagingOn) {
        ...
        for (var i = 0; i < len; i++)this.rowsCol[start + i].parentNode.removeChild(this.rowsCol[start + i]);
...
dhtmlXGridObject.prototype._massInsert = function (r, start, ind, skip) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (start)start.parentNode.insertBefore(ra, start); else
            par.appendChild(ra);
        anew.push(ra)
    }
    ;
...
dhtmlXGridObject.prototype.expandKids = function (curRow, sEv) {
    ...
    if (!r._loading && !sEv)if (!this.callEvent("onOpenStart", [r.id, -1])) return;
    ...
    ;
    this.setSizes();
    if (!r._loading)this.callEvent("onOpenEnd", [r.id, 1]);
    this._fixAlterCss()
};
dhtmlXGridObject.prototype.kidsXmlFile = "";
dhtmlXGridObject.prototype.sortTreeRows = function (col, type, order) {
    var amet = "getValue";
    if (this.cells5({parentNode:{grid:this}}, this.getColType(col)).getDate) {
        amet = "getDate";
        type = "camposAcionamento"
    }
    ;
    ...
    ;
    var funct = null;
    if (type == 'cus')funct = function (a, b) {
        return self._customSorts[col](a._sort, b._sort, order, a.id, b.id)
    };
    if (type == 'camposAcionamento')funct = function (a, b) {
        return (a._sort < b._sort ? neg : (a._sort == b._sort ? 0 : pos))
    };
    if (type == 'int')funct = function (a, b) {
        return (a._sort < b._sort ? neg : (a._sort == b._sort ? 0 : pos))
    };
    if (type == 'date')funct = function (a, b) {
        return (Date.parse(new Date(a._sort || "01/01/1900")) - Date.parse(new Date(b._sort || "01/01/1900"))) * pos
    };
...
dhtmlXGridObject.prototype._sortTreeRows = function (funct, id) {
    ...
    for (var i = 0; i < ar.length; i++) {
        if (ar[i].childs.length)this._sortTreeRows(funct, ar[i].id);
        ar[i].index = i
    }
};
...
dhtmlXGridObject.prototype._fixAlterCssTGR = function () {
    if (!this._realfake)this._h2.forEachChild(0, function (x) {
...
dhtmlXGridObject.prototype.moveRowUDTG = function (id, dir) {
    ...
    if (tar)tar.parentNode.insertBefore(nod, tar); else
...
eXcell_tree.prototype.setValue = function (valAr) {
    if (this.cell.parentNode.imgTag)return this.setLabel(valAr);
    ...
    row.label = valAr;
    var html = [_tgc.start];
    for (var i = 0; i < row.level; i++)html.push(_tgc.spacer);
    if (row.has_kids) {
        html.push(_tgc.plus);
        row.state = "plus"
    } else
...
dhtmlXGridObject.prototype._process_tree_xml = function (xml, top, pid) {
    ...
    ;
    if (main) {
        if (!rows.length)this._h2.change(pid, "state", dhtmlXGridObject._emptyLineImg); else if (pid != 0 && !this._srnd) {
        ...
        this._h2_to_buff();
        if (pid != 0 && this._srnd)this.openItem(pid); else {
            if (this.pagingOn)this._renderSort(); else
                this.render_dataset()
        }
        ;
        if (this.kidsXmlFile) {
            for (var i = 0; i < this._open.length; i++) {
                var r = this._h2.get[this._open[i]];
                if (r._xml_await)this.expandKids({idd:r.id})
            }
        }
        ;
        this._open = [];
...
dhtmlXGridObject.prototype._h2_to_buff = function (top) {
    ...
    for (var i = 0; i < top.childs.length; i++) {
        ...
        if (top.childs[i].state == "minus")this._h2_to_buff(top.childs[i])
    }
};
dhtmlXGridObject.prototype.render_row_tree = function (ind, id) {
    ...
    if (!r)return -1;
    if (r._parser) {
        if (this.rowsAr[r.idd] && this.rowsAr[r.idd].tagName == "TR")return this._h2.get[r.idd].buff = this.rowsBuffer[ind] = this.rowsAr[r.idd];
        var row = this._prepareRow(r.idd);
        this.rowsAr[r.idd] = row;
        if (!id)this.rowsBuffer[ind] = row;
...
dhtmlXGridObject.prototype._removeTrGrRow = function (node, x) {
    ...
    if (ind != -1 && ind !== this.undefined) {
        ...
        for (var i = 0; i < len; i++)if (this.rowsCol[i + ind])this.rowsCol[i + ind].parentNode.removeChild(this.rowsCol[i + ind]);
        if (this._fake)for (var i = 0; i < len; i++)this._fake.rowsCol[i + ind].parentNode.removeChild(this._fake.rowsCol[i + ind]);
...
dhtmlXGridObject.prototype.openItem = function (rowId) {
    ...
    if (!x)return;
    if (y.parent && y.parent.id != 0)this.openItem(y.parent.id);
    this.expandKids(x)
};
...
dhtmlXGridObject.prototype.addRow = function (new_id, text, ind, parent_id, img, child) {
    ...
    if (!row) {
        if (parent_id == 0)ind = this.rowsBuffer.length; else {
            ind = this.getRowIndex(parent_id) + 1;
            if (this._h2.get[parent_id].state == "minus")ind += this._getOpenLenght(parent_id, 0); else
...
dhtmlXGridObject.prototype.addRowAfter = function (new_id, text, sibl_id, img, child) {
    ...
    var ind = this.getRowIndex(sibl_id);
    if (ind == -1)this._skipInsert = true;
    if (this._h2.get[sibl_id].state == "minus")ind += this._getOpenLenght(sibl_id, 0) + 1; else ind++;
...
dhtmlXGridObject.prototype._copyTreeGridRowContent = function (frRow, from_row_id, to_row_id) {
    ...
    for (i = 0; i < frRow.cells.length; i++) {
        if (i != z)this.cells(to_row_id, i).setValue(this.cells(from_row_id, i).getValue())
        else
...
dhtmlXGridObject.prototype.deleteChildItems = function (rowId) {
    ...
    if (!z)return;
    while (z.childs.length)this.deleteRow(z.childs[0].id)
};
dhtmlXGridObject.prototype.getAllSubItems = function (rowId) {
    var str = [];
    var z = this._h2.get[rowId || 0];
    if (z)for (var i = 0; i < z.childs.length; i++) {
        str.push(z.childs[i].id);
        if (z.childs[i].childs.length)str = str.concat(this.getAllSubItems(z.childs[i].id).split(this.delim))
    }
    ;
...
dhtmlXGridObject.prototype.getSubItems = function (rowId) {
    var str = [];
    var z = this._h2.get[rowId || 0];
    if (z)for (var i = 0; i < z.childs.length; i++)str.push(z.childs[i].id);
    return str.join(this.delim)
};
